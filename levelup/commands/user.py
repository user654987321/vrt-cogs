...
async def view_profile_context(interaction: discord.Interaction, member: discord.Member):
    ...
    if not cog:
        return await interaction.response.send_message(_("Das Levelsystem ist nicht geladen!"), ephemeral=True)
    if member.bot and cog.db.ignore_bots:
        return await interaction.response.send_message(_("Bots können keine Profile haben!"), ephemeral=True)
    if not isinstance(interaction.user, discord.Member):
        return await interaction.response.send_message(_("Dieser Nutzer nicht nicht mehr auf diesem Server!"), ephemeral=True)
...
class User(MixinMeta):
    ...
    async def leveltop(
        ...
        """Schau dir das Leaderboard an

        **Arguments**
        `stat` - Die Statistik für die Bestenliste anzeigen, standardmäßig „exp“, kann aber eine der folgenden sein
        - `xp` - Experience
        - `level` - Level
        - `voice` - Voicetime
        - `messages` - Messages
        - `stars` - Stars
        `globalstats` - Sieh Dir die globale Bestenliste anstelle der Server-Bestenliste an
        `displayname` - Verwenden Sie Anzeigenamen anstelle von Benutzernamen
        """
        stat = stat.lower()
    ...
    async def role_group_leaderboard(self, ctx: commands.Context):
        """Schau dir das Rollen-Leaderboard an"""
        conf = self.db.get_conf(ctx.guild)
        if not conf.role_groups:
            return await ctx.send(_("Rollen Gruppen wurden noch nicht eingestellt!"))
        pages = await asyncio.to_thread(
        ...
        if not pages:
            return await ctx.send(_("Es sind noch keine Daten verfügbar!"))
        await DynamicMenu(ctx, pages).refresh()
    ...
    async def profile_data(self, ctx: commands.Context, user_id: int):
        ...
        conf = self.db.get_conf(ctx.guild)
        if not conf.enabled:
            return await ctx.send(_("Das Levelsystem ist auf diesen Server deaktiviert!"))
        if user_id not in conf.users:
            return await ctx.send(_("Dieser Benutzer hat noch keine Leveldaten!"))
    ...
    async def profile(self, ctx: commands.Context, *, user: t.Optional[discord.Member] = None):
        ...
        if not conf.enabled:
            ...
            if await self.bot.is_admin(ctx.author):
                txt += _("\nDu kannst es mit `{}` aktivieren").format(f"{ctx.clean_prefix}lset toggle")
        ...
        if user.bot and self.db.ignore_bots:
            return await ctx.send(_("Bots können keine Profile haben!"))
        ...
        if user.id == ctx.author.id and profile.show_tutorial:
            ...
            new_user_txt = _(
                "Willkommen beim Levelsystem!\n"
                "Verwende {} um deine Profileinstellungen und Bearbeitungsbefehle zu sehen!\n"
                "*Du kannst {} jederzeit verwenden um deine Profileinstellungen zu sehen*"
        ...
        try:
            ...
            if "Payload Too Large" in str(e):
                txt = _("Dein Profil ist zu groß um es zu senden!")
            else:
                txt = _("Beim generieren deines Profils ist ein Fehler aufgetreten!\n{}").format(str(e))
            await ctx.send(txt)
    ...
    async def prestige(self, ctx: commands.Context):
        ...
        Sobald du die Prestige-Level-Anforderung dieses Servers erreicht hast, kannst du
        ein Level und deine Erfahrung zurücksetzen, um ein Prestige-Level und die damit verbundenen Vorteile zu erhalten

        Wenn du beim Prestige-Aufstieg über Level und Erfahrungspunkte stehst, werden deine Erfahrungspunkte und Level übernommen
        """
        conf = self.db.get_conf(ctx.guild)
        if ctx.author.id not in conf.users:
            return await ctx.send(_("Du hast noch kein Level Daten!"))
        if not conf.prestigelevel or not conf.prestigedata:
            return await ctx.send(_("Presitge wurde auf diesem Server noch nicht konfiguriert!"))
        profile = conf.get_profile(ctx.author)
        if profile.level < conf.prestigelevel:
            return await ctx.send(_("Um Prestige zu erlangen musst du Level {} erreichen!").format(conf.prestigelevel))

        next_prestige = profile.prestige + 1
        if next_prestige not in conf.prestigedata:
            return await ctx.send(_("Du hast das maximale Presitge Level erreicht!"))

        pdata = conf.prestigedata[next_prestige]
        role = ctx.guild.get_role(pdata.role)
        if not role:
            return await ctx.send(_("Die Prestige Rolle für dieses Level existiert nicht mehr, bitte kontaktiere einen Admin!"))
        ...
        txt = _("Du hast Prestige {} erreicht!\n").format(f"**{next_prestige}**")
        added, removed = await self.ensure_roles(ctx.author, conf, _("Rang {} erreicht").format(next_prestige))
        ...
        if added:
            ...
            embed.add_field(name=_("Rollen hinzugefügt"), value=added_roles)
        if removed:
            removed_roles = humanize_list([r.mention for r in removed])
            embed.add_field(name=_("Rollen entfernt"), value=removed_roles)
    ...
    async def view_profile_settings(self, ctx: commands.Context):
        """Schaue dir deine Profil Einstellung an"""
        conf = self.db.get_conf(ctx.guild)
        profile = conf.get_profile(ctx.author)
        if not conf.use_embeds:
            ...
            desc = _("`Zeig den Nicknamen:   `{}\n").format(profile.show_displayname)
        color = ctx.author.color
        if color == discord.Color.default():
            color = await self.bot.get_embed_color(ctx)
        embed = discord.Embed(
            title=_("Deine Profil Einstellung"),
            description=desc,
            color=color,
        )
        ...
        if self.db.cache_seconds:
            embed.add_field(
                name=_("Cache Time"),
                value=_("Profildaten werden für {} Sekunden zwischengespeichert, das ist eine Einstellung von den Admins.").format(
        ...
        if not conf.use_embeds:
            embeds += [
                discord.Embed(
                    description=_("Namen Farbe: {}").format(f"**{str(profile.namecolor).title()}**"),
                    color=utils.string_to_rgb(profile.namecolor, as_discord_color=True),
                ),
                discord.Embed(
                    description=_("Statistik Farbe: {}").format(f"**{str(profile.statcolor).title()}**"),
                    color=utils.string_to_rgb(profile.statcolor, as_discord_color=True),
                ),
                discord.Embed(
                    description=_("Farbe der Levelleiste: {}").format(f"**{str(profile.barcolor).title()}**"),
    ...
    async def get_bg_path(self, ctx: commands.Context):
        """Bekomme den Hintergrund Speicherordner, für das Levelsystem"""
        txt = ""
    ...
    async def add_background(self, ctx: commands.Context, preferred_filename: str = None):
        ...
        **DISCLAIMER**
        ...
        if not content:
            return await ctx.send(_("Kein Bild in der Nachricht gefunden!"))
        valid = [".png", ".jpg", ".jpeg", ".gif", ".webp"]
        filename = content[0].filename
        if not filename.endswith(tuple(valid)):
            return await ctx.send(
                _("Das ist kein gültiges Format, es muss einer der folgenden Erweiterung sein: ") + humanize_list(valid)
            )
        for ext in valid:
        ...
        await ctx.send(_("Dein benutzerdefinierter Hintergrund wurde als {} gespeichert").format(f"`{filename}`"))
    ...
    async def remove_background(self, ctx: commands.Context, *, filename: str):
        ...
        for path in self.custom_backgrounds.iterdir():
            ...
            return await ctx.send(_("Kein Hintergrund mit diesem Namen gefunden!"))
        msg = await ctx.send(_("Bist du sicher, dass du {} entfernen willst?").format(f"`{path}`"))
        yes = await utils.confirm_msg(ctx)
        if not yes:
            return await msg.edit(content=_("Abgebrochen"))
        path.unlink()
        await msg.edit(content=_("Der Hintergrund {} wurde entfernt").format(f"`{path}`"))
    ...
    async def remove_font(self, ctx: commands.Context, *, filename: str):
        """"Entferne eine Standard-Schriftart aus dem Cog Ordner"""
        for path in self.custom_fonts.iterdir():
            if filename.lower() in path.name.lower():
                break
        else:
            return await ctx.send(_("Keine Schriftart mit diesen Namen gefunden!"))
        msg = await ctx.send(_("Bist du sicher, dass du {} löschen möchtest?").format(f"`{path}`"))
        yes = await utils.confirm_msg(ctx)
        if not yes:
            return await msg.edit(content=_("Cancelled"))
        path.unlink()
        await msg.edit(content=_("Die Schriftart {} wurde gelöscht").format(f"`{path}`"))

    @set_profile.command(name="backgrounds")
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.bot_has_permissions(attach_files=True)
    async def view_all_backgrounds(self, ctx: commands.Context):
        """Schau dir alle verfügbaren Hintergründe an"""
        conf = self.db.get_conf(ctx.guild)
        if conf.use_embeds:
            return await ctx.send(_("Profilbilder sind deaktiviert, diese Einstellung hat keine Auswirkung!"))
        ...
        async with ctx.typing():
            ...
            txt = _("Hier sind alle verfügbaren hintergründe!\nDu kannst {} verwenden um dein Hintergrund zu setzen").format(
    ...
    async def view_fonts(self, ctx: commands.Context):
        """Schau dir die verfügbaren Schriftarten an, die du verwenden kannst"""
        conf = self.db.get_conf(ctx.guild)
        if conf.use_embeds:
            return await ctx.send(_("Profilbilder sind deaktiviert, diese Einstellung hat keine Auswirkung!"))
        ...
        async with ctx.typing():
            file = await asyncio.to_thread(_run)
            txt = _("Hier sind alle verfügbaren Schriftarten!\nDu kannst {} verwenden um deine Schriftart zu setzen").format(
    ...
    async def toggle_profile_style(self, ctx: commands.Context, style: t.Literal["default", "runescape"]):
        """
        Ändere dein Profil Stil

        - `default` ist der Standard style, sehr anpassbar
        - `runescape` ist ein Profil im Runescape-Stil, weniger anpassbar, aber nostalgischer
        - (WIP) - Soon™
        ...
        if style not in const.PROFILE_TYPES:
            txt = _("Das ist kein gültiger Profilstiel, Bitte verwende eines der folgenden: {}").format(
        ...
        profile.style = style
        self.save()
        await ctx.send(_("Dein Profiltyp wurde auf {} gesetzt").format(style.capitalize()))

    @set_profile.command(name="shownick")
    async def toggle_show_nickname(self, ctx: commands.Context):
        """Wechsel ob dein Anzeigename oder Benutzername in deinem Profil angezeigt werden soll"""
        conf = self.db.get_conf(ctx.guild)
        profile = conf.get_profile(ctx.author)
        profile.show_displayname = not profile.show_displayname
        self.save()
        txt = (
            _("Dein Anzeigename wird nun in deinem Profil angezeigt!")
            if profile.show_displayname
            else _("Dein Nutzername wir in deinem Profil angezeigt!")
        )
        await ctx.send(txt)
    ...
    async def set_name_color(self, ctx: commands.Context, *, color: str):
        """
        Setze eine Farbe für deinen Namen

        Für eine bestimmte Farbe, verwende: **[Google's hex color picker](https://htmlcolorcodes.com/)**

        Verwende `default` um bei jedem öffnen deines Profils eine zufällige Farbe zu erhalten
        """
        conf = self.db.get_conf(ctx.guild)
        if conf.use_embeds:
            return await ctx.send(_("Profilbilder sind deaktiviert, diese Einstellung hat keine Auswirkung!"))
        ...
        if profile.style in const.STATIC_FONT_STYLES:
            return await ctx.send(_("Du kannst die Farbe deines Namens mit deinem aktuellen Profilstiel nicht ändern!"))
        if color == "default":
            profile.namecolor = None
            self.save()
            return await ctx.send(_("Die Farbe deines Names wurde auf Zufällig gesetzt!"))
        try:
            rgb = utils.string_to_rgb(color)
        except ValueError:
            file = discord.File(imgtools.COLORTABLE)
            return await ctx.send(
                _("Das ist keine gültige Farbe, bitte verwende einen gültigen Namen, Integer oder Hex-Farbcode."), file=file
            )
        embed = discord.Embed(
            description=_("Namen Farbe wurde zu {} geändert!").format(f"`{color}`"),
    ...
    async def set_stat_color(self, ctx: commands.Context, *, color: str):
        """
        Wähle eine Farbe für deine Server-Statistiken
        ...
        if color == "default":
            ...
            self.save()
            return await ctx.send(_("Deine Statistikenfarbe wurde auf zufällig gesetzt!"))
        try:
            rgb = utils.string_to_rgb(color)
        except ValueError:
            file = discord.File(imgtools.COLORTABLE)
        ...
        embed = discord.Embed(
            description=_("Die Statistiken-Farbe wurde zu {} geändert!").format(f"`{color}`"),
            color=discord.Color.from_rgb(*rgb),
        )
    ...
    async def set_levelbar_color(self, ctx: commands.Context, *, color: str):
        """
        Setze eine Farbe für deinen Levelleiste
        ...
        profile = conf.get_profile(ctx.author)
        if profile.style in const.STATIC_FONT_STYLES:
            return await ctx.send(_("Du kannst die Farbe deiner Levelleiste mit deinem aktuellen Profilstiel nicht ändern!"))
        if color == "default":
            profile.barcolor = None
            self.save()
            return await ctx.send(_("Levelleistenfarbe wurde zufällig gesetzt!"))
        try:
            rgb = utils.string_to_rgb(color)
        except ValueError:
            file = discord.File(imgtools.COLORTABLE)
        ...
        embed = discord.Embed(
            description=_("Levelleistenfarbe wurde auf {} gesetzt!").format(f"`{color}`"),
            color=discord.Color.from_rgb(*rgb),
        )
    ...
    async def set_user_background(
        ...
        """
        Setze einen Hintergrund für dein Profil

        Dies wird deinen Profilbanner mit dem Hintergrund ersetzen

        **WARNUNG**
        Der Standart Profilstiel hat ein Seitenverhältnis von 21:9 (1050 - 450 pixel).
        Hochformatbilder werden zugeschnitten.

        Tipp: Google "dual monitor backgrounds" das liefert gute Ergebnisse für die richtigen Bilder

        Hier sind einige gute Orte zum Suchen.
        ...
        **Weitere Optionen**
         - Lassen Sie „URL“ leer oder geben Sie „default“ an, um die Verwendung Ihres Profilbanners wiederherzustellen (oder ein zufälliges Banner, wenn Sie keins haben).
         - `random` wählt jedes Mal zufällig aus einem Pool von Standardhintergründen aus
         - `filename` führe `[p]mypf backgrounds` aus, um die Standardoptionen anzuzeigen, die du verwenden kannst, indem du ihren Dateinamen angibst.
        """
        conf = self.db.get_conf(ctx.guild)
        if conf.use_embeds:
            txt = _("Profilbilder sind deaktiviert, diese Einstellung hat keine Auswirkung!")
            return await ctx.send(txt)
        profile = conf.get_profile(ctx.author)
        if profile.style in const.STATIC_FONT_STYLES:
            return await ctx.send(_("Du kannst die Farbe deines Namens mit deinem aktuellen Profilstiel nicht ändern!"))

        cached_txt = _("\n\nProfile werden für {} Sekunden gespeichert somit sind Änderungen möglicherweise nicht sofort sichtbar").format(
            self.db.cache_seconds
        )
        ...
        if url and url == "random":
            ...
            self.save()
            txt = _("Dein Profilhintergrund wurde auf zufällig gesetzt!")
            if self.db.cache_seconds:
                txt += cached_txt
            return await ctx.send(txt)
        if url and url == "default":
            ...
            self.save()
            txt = _("Dein Profilhintergrund wurde auf Standard gesetzt")
            if self.db.cache_seconds:
                txt += cached_txt
            return await ctx.send(txt)
        ...
        if not url and not attachments:
            if profile.background == "default":
                return await ctx.send(_("Du musst eine URL, einen Dateinamen angeben oder eine Datei anhängen"))
            else:
                profile.background = "default"
                self.save()
                txt = _("Dein Hintergrund wurde auf Standard zurückgesetzt")
                if self.db.cache_seconds:
                    txt += cached_txt
                return await ctx.send(txt)
        ...
        if url is None:
            if attachments[0].size > ctx.guild.filesize_limit:
                return await ctx.send(_("Dieses Bild ist zu groß für das Server upload limit!"))
            ...
            try:
                ...
                profile.background = "default"
                return await ctx.send(_("Das ist kein gültiger Hintergrund!\n{}").format(str(e)))
            self.save()
            txt = _("Dein Profilhintergrund wurde gesetzt!")
            if self.db.cache_seconds:
                txt += cached_txt
            return await ctx.send(txt)
        ...
        if url.startswith("http"):
            ...
            try:
                ...
                profile.background = "default"
                return await ctx.send(_("Dieses Bild ist kein gültiger Profil Hintergrund!\n{}").format(str(e)))
            self.save()
            txt = _("Dein Profilhintergrund wurde gesetzt!")
            if self.db.cache_seconds:
                txt += cached_txt
        ...
        for path in backgrounds:
            ...
            return await ctx.send(_("Es wurde kein Hintergrund mit dem Namen gefunden!"))
        file = discord.File(path)
        profile.background = path.stem
        self.save()
        txt = _("Dein Profilhintergrund wurde zu {} gesetzt").format(f"`{path.name}`")
        if self.db.cache_seconds:
            txt += cached_txt
        await ctx.send(txt, file=file)
    ...
    async def set_user_font(self, ctx: commands.Context, *, font_name: str):
        """
        Setze eine Schriftart von deinem Profil

        Verwende `[p]myprofile fonts` um verfügbare Schriftarten zu sehen
        Verwende `default` für das `font_name` Argument um es zur Standard Schriftart zurückzusetzen
        """
        conf = self.db.get_conf(ctx.guild)
        if conf.use_embeds:
            txt = _("Profilbilder sind deaktiviert, diese Einstellung hat keine Auswirkung!")
        ...
        if font_name == "default":
            ...
            self.save()
            return await ctx.send(_("Deine Schriftart wurde auf Standard zurückgesetzt!"))
    ...
    async def set_user_blur(self, ctx: commands.Context):
        """
        Schalte einen leichten unschärfe Effekt für den Hintergrund ein wo der Text eingeblendet wird.
        """
        conf = self.db.get_conf(ctx.guild)
        if conf.use_embeds:
            txt = _("Profilbilder sind deaktiviert, diese Einstellung hat keine Auswirkung!")
        ...
        self.save()
        txt = _("Deine Profilunschärfe wurde auf {} gesetzt").format(_("Enabled") if profile.blur else _("Disabled"))
        await ctx.send(txt)
